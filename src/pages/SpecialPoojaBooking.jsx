import React from "react";
import { motion } from "framer-motion";

export default function SpecialPoojaBookingPage() {
  return (
    <div className="min-h-screen bg-transparent relative overflow-x-hidden">
           <div className="absolute top-[-90px] h-[250px] opacity-[1] left-[0px] z-30">
        <img
          src="/assets/flowers.png"
          alt="flower"
          className="w-[100%] h-[100%]"
        />
      </div>
      <div className="absolute  top-[-90px] h-[250px] opacity-[1] right-[0px] z-30">
        <img
          src="/assets/r-flowers.png"
          alt="flower"
          className="w-[100%] h-[100%] "
        />
      </div>
      <div className="absolute w-[600px] h-[600px] bottom-[20%] opacity-[.2] right-[-350px] z-8">
        <img
          src="/assets/rotatebg.png"
          alt=""
          className="w-[100%] h-[100%] spin-slow"
        />
      </div>
      {/* Welcome Header */}
      <section className="bg-orange-50/95 backdrop-blur-sm py-12 px-4">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="text-center"
        >
          <h1 className="text-4xl font-bold text-gray-900 mb-6">
            🙏 Jai Sri Ram!
                <br />
             Welcome to Sri Bhaktha Hanuman Temple Special Puja Booking
            🙏
          </h1>

          <div className="max-w-3xl mx-auto bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/20 mb-8">
            <p className="text-lg text-gray-700 mb-4 leading-relaxed">
              We are delighted to help you book your spiritual ceremonies and
              connect with the divine through traditional pujas. Please fill out
              the form below with your preferred puja details and timing.
            </p>
            <p className="text-xl font-semibold text-orange-700 mb-2">
              Your Devotion = Temple's Blessing!
            </p>
            <p className="text-gray-600">
              Our temple coordinator will contact you soon to confirm your
              booking details.
            </p>
          </div>
        </motion.div>
      </section>

      {/* Google Form Embed */}
      <div className="py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.2 }}
          >
            <div className="bg-white/90 backdrop-blur-sm shadow-xl rounded-lg p-1 md:p-2">
              <iframe
                src="https://docs.google.com/forms/d/e/1FAIpQLSdo6dM4zl_Q3BwXKY0WUNJpHet8ZA2XEQjHIr2pt2uYzVK6rg/viewform?usp=dialog"
                width="100%"
                height="1500"
                frameBorder="0"
                marginHeight="0"
                marginWidth="0"
                className="rounded-lg"
              >
                Loading…
              </iframe>
            </div>
          </motion.div>
        </div>
      </div>
    </div>
  );
}
